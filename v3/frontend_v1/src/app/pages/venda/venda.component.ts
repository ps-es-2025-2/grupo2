...
>>>>>>> 7a918497ebee151fabee2fa8e53dade07b3544a5
  ...
  // Atualiza o saldo do caixa no localStorage após uma venda bem-sucedida
  private atualizarSaldoCaixaAposVenda(valorVenda: number): void {
    try {
      const caixaLocalRaw = localStorage.getItem('caixa_local');
      if (caixaLocalRaw) {
        const caixaLocal = JSON.parse(caixaLocalRaw);
        caixaLocal.saldo = (caixaLocal.saldo || 0) + valorVenda;
        localStorage.setItem('caixa_local', JSON.stringify(caixaLocal));
        console.log('Saldo do caixa atualizado:', caixaLocal.saldo);
      }
    } catch (error) {
